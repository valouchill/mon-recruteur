...
# -------------------------------------------------------------------
# Ce build améliore drastiquement la robustesse du scoring vs v15 :
# - MOTEURS : Groq (LLM), Ollama local (gratuit), Règles seules (gratuit)
# - HYBRIDE : combinaison pondérée Règles↔IA, cap dur si must‑have manquant
# - RÈGLES : parsing compétences (synonymes), années d'expérience, soft skills,
#            contraintes (langue CEFR, diplôme, remote, localisation)
# - TRANSPARENCE : détail des manquants, points forts explicites
# - PONDÉRATIONS : sliders Tech/Exp/Soft/Fit + poids des règles (alpha)
# - DÉDOUBLONNAGE + ANONYMISATION conservés
...
st.markdown(
    ...
    .kpi-card.primary::before{background:var(--primary)} .kpi-card.success::before{background:var(--success)} .kpi-card.warning::before{background:var(--warning)}
    .kpi-val{font-size:1.6rem;font-weight:700} .kpi-label{font-size:.8rem;color:#475569;text-transform:uppercase;font-weight:600}
    ...
    .score-high{background:linear-gradient(135deg,#059669,#065f46)} .score-mid{background:linear-gradient(135deg,#9a3412,#7c2d12)} .score-low{background:linear-gradient(135deg,#b91c1c,#7f1d1d)}
    ...
    .match{background:#ecfdf5;border-color:#34d399;color:#065f46} .missing{background:#fff7ed;border-color:#fdba74;color:#7c2d12;text-decoration:line-through}
    </style>
    """,
...
class Scores(BaseModel):
    global_: conint(ge=0, le=100) = Field(0, alias="global"); tech: conint(ge=0, le=100) = 0; experience: conint(ge=0, le=100) = 0; soft: conint(ge=0, le=100) = 0; fit: conint(ge=0, le=100) = 0
...
class CandidateData(BaseModel):
    infos: Infos = Infos(); scores: Scores = Scores(); salaire: Salaire = Salaire(); analyse: Analyse = Analyse(); competences: Competences = Competences(); historique: List[HistoriqueItem] = []; entretien: List[QuestionItem] = []
...
SKILL_SYNONYMS = {
    "python":["python"],"java":["java"],"javascript":["javascript","js","node.js","nodejs"],"typescript":["typescript","ts"],
...
def parse_must_haves(criteria: str) -> Dict[str, Any]:
    ...
    for t in tokens:
        # années
        m = YEARS_RE.search(t)
        if m: must["min_years"] = max(int(m.group(1)), must["min_years"] or 0); continue
        # langue CEFR
        m2 = CEFR_RE.search(t)
        if m2: must["lang"] = m2.group(1).upper(); continue
        # diplômes
    ...
    # dedupe
    must["skills"] = list(dict.fromkeys(must["skills"]))
    return must
...
def compute_rule_based(job: str, cv: str, criteria: str, weights: Tuple[float,float,float,float]) -> Dict[str, Any]:
    ...
    gated = bool(missing)
    if gated: global_rule = min(global_rule, 49)
...
Schéma JSON: infos, scores, salaire, competences, analyse, historique, entretien (identiques à v15).
"""
...
def analyze_candidate(job: str, cv: str, criteria: str, file_id: str, provider: str, model_name: str, mode: str, weights: Tuple[float,float,float,float], alpha_rules: float) -> Optional[Dict[str, Any]]:
    ...
    if rules["competences"].get("manquant"): combined["global"] = min(combined["global"], 49)
...
def anonymize_infos(i: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ret.get("tel"):
        t = re.sub(r"\D", "", ret["tel"]) ; ret["tel"] = "✱✱✱✱ " + (t[-4:] if len(t)>=4 else "✱✱✱✱")
...
def process_single_cv(file_obj, job_text: str, criteria: str, weights: Tuple[float,float,float,float], provider: str, model_name: str, mode_choice: str, alpha_rules: int) -> Optional[Dict[str, Any]]:
    ...
    if len(cv_text) < 50:
        st.warning(f"⚠️ {file_obj.name}: contenu insuffisant."); return None
...
if not results:
    ...
    st.write(""); left, mid, right = st.columns([2,1,2])
    with left:
    ...
    for idx, d in enumerate(filtered):
        i, s = d["infos"], d["scores"]
        score_class = "score-high" if s.get("global",0) >= 75 else ("score-mid" if s.get("global",0) >= 50 else "score-low")
        with st.expander(f"{('🌟' if score_class=='score-high' else '⚡' if score_class=='score-mid' else '⚠️')} {i.get('nom','Candidat')} — {s.get('global',0)}%", expanded=(idx==0)):
            st.markdown(f"""
                <div style='display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:12px;border-bottom:1px solid #f1f5f9;margin-bottom:12px;'>
                    <div>
                        <h3 style='margin:0'>{i.get('nom','Candidat')}</h3>
                        <div style='color:#334155'>{i.get('poste_actuel','')} • {i.get('ville','')}</div>
                        <div style='margin-top:10px;'>
                            <span class='skill-tag'>📧 {i.get('email','')}</span>
                            <span class='skill-tag'>📞 {i.get('tel','')}</span>
                            <span class='skill-tag'>🔗 {i.get('linkedin','')}</span>
                        </div>
                    </div>
            ...
            cA, cB = st.columns(2)
            with cA:
                for f in (d.get("analyse",{}).get("points_forts",[]) or [])[:5]: st.write("✅ ", f)
            with cB:
                for r in (d.get("analyse",{}).get("points_faibles",[]) or [])[:5]: st.write("❗ ", r)
            st.divider()
            ...
            with st.expander("🎯 Guide d'entretien"):
                for q in d.get("entretien", [])[:6]:
                    theme = q.get("theme","Général"); st.markdown(f"**{theme}** — {q.get('question','')}\n\n> Attendu : _{q.get('attendu','')}_")
